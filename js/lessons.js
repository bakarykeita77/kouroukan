           ...
           function lessonContent() {
            ...
            //    var nasalisations_cochees = JSON.parse(sessionStorage.getItem('nasalisations_cochees'));
               ...
               if(course_id == "apprentissage") {
                   if(niveau_actif == 1) lesson_content = lettresCochees();
                   if(niveau_actif == 2) lesson_content = syllabesSimples();
                   if(niveau_actif == 3) lesson_content = syllabesTonifiees();
                   if(niveau_actif == 4) lesson_content = chiffresDeBase();
               }
               
               if(course_id == "exercice") {
                   if(niveau_actif == 1) lesson_content = malaxer(lettresCochees());
                   if(niveau_actif == 2) lesson_content = malaxer(syllabesSimples());
                   if(niveau_actif == 3) lesson_content = malaxer(syllabesTonifiees());
                   if(niveau_actif == 4) lesson_content = malaxer(chiffresDeBase());
               }
               
               if(course_id == "pratique") {
                   if(niveau_actif == 1) lesson_content = [];
                   if(niveau_actif == 2) lesson_content = [];
                   if(niveau_actif == 3) lesson_content = [];
                   if(niveau_actif == 4) lesson_content = [];
               }
